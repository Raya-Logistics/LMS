...
export class TopbarComponent implements OnInit {
  ...
  private eventService: EventService, public languageService: LanguageService, private modalService: NgbModal,
    ...
    private router: Router, private TokenStorageService: TokenStorageService,private signalRService: SignalRService, private _apihandler: ApihandlerService
  ) { }

  ngOnInit(): void {
    this.userData = this.TokenStorageService.getUser();
    this.userInfoData = this.TokenStorageService.getUserInfo();
...
*/
  ...
  */
  ...
  logout() {
    ...
    //         this.signalRService.UpdatedLogingUsersListener((users) => {
  ...
  scanWhQueueBarcodeOperation() {
    console.log(this.scanWhQueueBarcode, "  testt")
  ...
  scanTransactionNumber() {
    if(this.transactionNumber && this.transactionNumber.split("-")?.length == 2) {
      this._apihandler.GetItem(`${GlobalComponent.BASE_API}/OperationManager/CheckValidTransactionNumber?transactionNumber=${this.transactionNumber.trim()}`).subscribe({
        next:(response) => {
          if(response.success) {
            if(response.returnObject.handlingType == "IN"){
              this.router.navigate(['handlingIn',response.returnObject.transactionNumber])

            }else {
              this.router.navigate(['handlingOut',response.returnObject.transactionNumber])
      ...
      Swal.fire({
        icon: 'error', // You can change this to 'success', 'warning', etc.
        title: 'خطأ...',
        text: "You Must Enter Valid Transaction Number, يجب عليك ادخال رقم عمليه صحيح",
        // footer: '<a href="">Why do I have this issue?</a>', // Optional footer if needed
        confirmButtonText: 'close',
        confirmButtonColor: '#3085d6'
      });
