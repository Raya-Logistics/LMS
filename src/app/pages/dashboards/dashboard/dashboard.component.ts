...
export class DashboardComponent implements OnInit {
  ...
  constructor(public toastService: ToastService, private _apihandler:ApihandlerService,
  ...
  ngOnInit(): void {
    ...
    */
    ...
    this._apihandler.GetItem(`${GlobalComponent.BASE_API}/StockDetails/YardDashboard?startDate=${startDate.toISOString()}&endDate=${currentDate}`).subscribe({
      next: (response) => {
        this.fetchData(response.returnObject);
      }
    });
  ...
  onDateChange(selectedDates: string[]): void {
    const formatDateAsLocal = (date: string): string => {
    ...
    const fromDate = this.currentDate?.from ? formatDateAsLocal(this.currentDate.from) : null;
    const toDate = this.currentDate?.to ? formatDateAsLocal(this.currentDate.to) : null;
    ...
    this.spinner.show();
    this._apihandler.GetItem(`${GlobalComponent.BASE_API}/StockDetails/YardDashboard${queryString}`).subscribe({
      next: (response) => {
 ...
 */
  ...
  * Fetches the data
  */
  private fetchData(mainObject:any) {
    this.spinner.hide();
    this.BestSelling = BestSelling;
    this.TopSelling = TopSelling;
    // this.Recentelling = mainObject.resultStocksFreeTime[0].stockInFreeTime;
    this.StockIn = mainObject.resultStocksFreeTime[0].stockInFreeTime;
    this.StockOut = mainObject.resultStocksFreeTime[0].stockOutFreeTime;
    this.statData = mainObject.resultStocksCount;
  }
