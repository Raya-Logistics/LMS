...
const BaseURL = 'https://localhost:7202/api/PositionLevel';
const BaseURL_GetAll = 'get-all-positionLevel';
const BaseURL_GetAllPaged = 'get-all-positionLevel-pagination';
const BaseURL_GetById = 'get-positionLevel-by-id';
const BaseURL_Post = 'Create-positionLevel';
const BaseURL_Update = 'edit-positionLevel';
const BaseURL_Delete = 'deleted-positionLevel';
const BaseURL_PostAll = 'Create-list-positionLevel';
@Component({
  selector: 'app-positionlevel',
  templateUrl: './positionlevel.component.html',
  styleUrls: ['./positionlevel.component.scss']
})
export class PositionlevelComponent {
  ...
  ngOnInit(): void {
   this.dataSource = this._apihandler.getStore(`${GlobalComponent.BASE_API}/PositionLevel`, BaseURL_GetAll,BaseURL_GetById,BaseURL_Post,
    BaseURL_Update,BaseURL_Delete)
    this.breadCrumbItems = [
        { label: 'Home' },
        { label: 'Position Level', active: true }
    ];
    // this.internalPermission = this.authentication.getInternalPermissions(this.pagesname.PositionLevel)
    this.authentication.getInternalPermissionss(this.pagesname.PositionLevel).subscribe({
      next:(permissions) => {
    ...
    for (const file of files) {
      this._filehandler.getjsonDataFromFile(file)
        .then(jsonData => {
          ...
          this._apihandler.AddItem(`${GlobalComponent.BASE_API}/PositionLevel/${BaseURL_PostAll}`, jsonData).subscribe({
            next:(response)=>{
              if(response.success)
